...
| Kubernetes (needs a KinD cluster, not the Compose stack) | `bash scripts/kind-up.sh && bash scripts/kind-load-images.sh && bash scripts/kind-deploy.sh && bash scripts/kind-smoke-test.sh` after `bash scripts/build-images.sh` | same smoke test through `kubectl port-forward`, then `rides`/`ride_assignments` checked with `psql` and the notification counter checked over HTTP; `bash scripts/kind-down.sh` deletes the cluster |
