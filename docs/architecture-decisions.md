...
The outbox prevents event loss and accepts writes while Redis is unavailable. Delivery is at-least-once rather than exactly-once: a relay crash after `XADD` but before marking the row published can create a duplicate with the same envelope ID. Consumers therefore need idempotent state transitions. Relay progress is monotonic within a batch: a publication failure stops the batch, commits the rows already delivered, and records the failure outside the transaction, so an undeliverable record cannot replay the events ahead of it on every poll. The one remaining exception is a failure while recording publication, because PostgreSQL forbids committing an aborted transaction. Holding row locks during publication keeps this implementation compact; a higher-throughput system could use leases and partitioned relay workers.
